...
class MapartController extends Component {
  ...
  handleGetPDNPaletteClicked = () => {
    this.setState({ showingPaletteFormat: true });
  };

  handlePaletteFormatCancel = () => {
    this.setState({ showingPaletteFormat: false });
  };

  handleExportPalette = (format) => {
    ...
    for (const [selectedBlock_colourSetId, selectedBlock_blockId] of Object.entries(selectedBlocks)) {
      if (selectedBlock_blockId !== "-1") {
        ...
        for (const toneKeyToExport of toneKeysToExport) {
          colourRGBs.push([Number(colours[toneKeyToExport][0]), Number(colours[toneKeyToExport][1]), Number(colours[toneKeyToExport][2])]);
        }
      }
    }
    ...
    if (format === "paintnet") {
      const headerText =
        ...
        ([MapModes.MAPDAT.staircaseModes.ON_UNOBTAINABLE.uniqueId, MapModes.MAPDAT.staircaseModes.FULL_UNOBTAINABLE.uniqueId].includes(optionValue_staircasing)
          ...
          : "disabled") +
        "\n";
      const maxColoursPerPalette = 96; // Paint.NET palette hard limit
      for (let chunkIndex = 0; chunkIndex < colourRGBs.length; chunkIndex += maxColoursPerPalette) {
        const chunk = colourRGBs.slice(chunkIndex, chunkIndex + maxColoursPerPalette);
        const paletteText = headerText + chunk.map(([r, g, b]) => `FF${this.toHex2(r)}${this.toHex2(g)}${this.toHex2(b)}\n`).join("");
        const fileIndex = chunkIndex / maxColoursPerPalette + 1;
        const filename = colourRGBs.length > maxColoursPerPalette ? `MapartcraftPalette_${fileIndex}.txt` : "MapartcraftPalette.txt";
        this.downloadBlobFile(new Blob([paletteText], { type: "text/plain" }), filename);
      }
      if (colourRGBs.length > maxColoursPerPalette) {
        const fileCount = Math.ceil(colourRGBs.length / maxColoursPerPalette);
        alert(
          `${getLocaleString("BLOCK-SELECTION/PRESETS/DOWNLOAD-WARNING-MAX-COLOURS-1")}${colourRGBs.length.toString()} ${getLocaleString(
            "BLOCK-SELECTION/PRESETS/DOWNLOAD-SPLIT-FILES"
          )
            .replace("{0}", fileCount.toString())
            .replace("{1}", maxColoursPerPalette.toString())}`
        );
      }
    } else if (format === "gpl") {
      let gplText = "GIMP Palette\nName: MapartCraft\nColumns: 16\n#\n";
      gplText += colourRGBs.map(([r, g, b]) => `${r}\t${g}\t${b}\tMapartCraft\n`).join("");
      this.downloadBlobFile(new Blob([gplText], { type: "text/plain" }), "MapartcraftPalette.gpl");
    } else if (format === "act") {
      const actData = new Uint8Array(768); // 256 colours * 3 bytes
      colourRGBs.slice(0, 256).forEach(([r, g, b], i) => {
        actData[i * 3] = r;
        actData[i * 3 + 1] = g;
        actData[i * 3 + 2] = b;
      });
      this.downloadBlobFile(new Blob([actData], { type: "application/octet-stream" }), "MapartcraftPalette.act");
    }
  };
  ...
  render() {
    ...
    return (
      <div className="mapartController">
        ...
        {this.state.showingPaletteFormat && (
          <div
            style={{
              position: "fixed",
              inset: 0,
            ...
            onClick={this.handlePaletteFormatCancel}
          >
            <div
              style={{ backgroundColor: "#222", padding: "1em", borderRadius: "0.5em", maxWidth: "90%" }}
              onClick={(e) => e.stopPropagation()}
            >
              <p>{getLocaleString("BLOCK-SELECTION/PRESETS/DOWNLOAD-FORMAT-TITLE")}</p>
              <div style={{ display: "flex", flexDirection: "column", gap: "0.5em" }}>
                <button type="button" onClick={() => this.handleExportPalette("paintnet")}>
                  {getLocaleString("BLOCK-SELECTION/PRESETS/DOWNLOAD-FORMAT-PAINTNET")}
                </button>
                <button type="button" onClick={() => this.handleExportPalette("gpl")}>
                  {getLocaleString("BLOCK-SELECTION/PRESETS/DOWNLOAD-FORMAT-GPL")}
                </button>
                <button type="button" onClick={() => this.handleExportPalette("act")}>
                  {getLocaleString("BLOCK-SELECTION/PRESETS/DOWNLOAD-FORMAT-ACT")}
                </button>
                <button type="button" onClick={this.handlePaletteFormatCancel}>
                  {getLocaleString("BLOCK-SELECTION/PRESETS/DOWNLOAD-FORMAT-CANCEL")}
                </button>
              </div>
            </div>
          </div>
